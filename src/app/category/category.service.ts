...
export class CategoryService {
  ...
  getCategoryList(): Observable<CategorySummary[]> {
    return this.http.get<CategorySummary[]>(`${this.apiUrl}/list`, { headers: this.getHeaders() });
  }

  getCategoryListForFamily(familyId: string): Observable<CategorySummary[]> {
    return this.http.get<CategorySummary[]>(`${this.apiUrl}/list/for-family/${familyId}`, { headers: this.getHeaders() });
  }

  createCategory(category: Category, familyId: string): Observable<any> {
    const params = new HttpParams().set('familyId', familyId);
    return this.http.post<any>(this.apiUrl, category, { params, headers: this.getHeaders() });
  }

  updateCategory(categoryId: string, category: Category, familyId: string): Observable<any> {
    const params = new HttpParams().set('familyId', familyId);
    return this.http.put<any>(`${this.apiUrl}/${categoryId}`, category, { params, headers: this.getHeaders() });
  }
